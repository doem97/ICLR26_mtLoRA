#!/bin/bash
# Block Position Ablation: FFN Only
# Dataset: Flan V2 Subset -> BBH Evaluation
# 
# Block adapter applied to FFN blocks only (parallel to MLP)
# 
# ============================================================
# NOTE: Similar experiment exists at exp/1125/bbh_ablation/4_block_adapter.sh
# This is kept for completeness and consistent output structure.
# ============================================================

set -e

echo "========================================"
echo "[1b] Block Adapter: FFN Only"
echo "========================================"
echo "Position: Parallel to FFN blocks"
echo "Style: lowrank (no activation)"
echo "========================================"
...
OUTPUT_DIR="./output/bbh/1_block_position/1b_ffn_only"
LOG_DIR="./logs/bbh/1_block_position/1b_ffn_only"
...
echo "  Block Adapter: type=$BLOCK_ADAPTER_TYPE, style=$BLOCK_ADAPTER_STYLE"
echo "  LoRA: rank=$LORA_RANK, experts=$LORA_NUMS"

accelerate launch --num_processes $NPROC_PER_NODE --main_process_port 29611 \
    train.py \
