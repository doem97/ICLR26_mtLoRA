...
from .tuners.adalora import AdaLoraModel
from .tuners.mmoelora import MMOELoraModel
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig):  # casualLM, LoraConfig
        ...
        else:  # Choose the appropriate model class based on peft_type
            if self.peft_config.peft_type == PeftType.ADALORA:
                adapter_name = "default"
                self.base_model = AdaLoraModel(model, {adapter_name: peft_config}, adapter_name)
            elif self.peft_config.peft_type == PeftType.MMOELORA:
                adapter_name = "default"
                self.base_model = MMOELoraModel(model, {adapter_name: peft_config}, adapter_name)
            elif self.peft_config.peft_type == PeftType.MMOELORAS:
                adapter_name = "default"
                self.base_model = MMOELoraModelS(model, {adapter_name: peft_config}, adapter_name)
            else:  # Standard LoRA and HydraLoRA
    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        output_state_dict = get_peft_model_state_dict(self, kwargs.get("state_dict"))
    ...
    def _setup_prompt_encoder(self):
        ...
        for name, module in self.base_model.named_children():
            ...
            if isinstance(module, PreTrainedModel) and transformer_backbone is None:
