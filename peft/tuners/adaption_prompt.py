...
from HydraLoRA.peft.utils.config import PeftConfig, PeftType
from HydraLoRA.peft.utils.other import (
    _freeze_adapter,
    _get_submodules,
)
...
class AdaptionPromptConfig(PeftConfig):
    ...
    adapter_layers: int = field(
        default=None,
        metadata={"help": "Number of adapter layers (from the top)"},
    )
...
class AdaptionPromptModel(nn.Module):
    ...
    def add_adapter(
        self, adapter_name: str, config: AdaptionPromptConfig
    ) -> None:
    ...
    def set_adapter(self, adapter_name: str) -> None:
        ...
        if adapter_name not in self._configs:
            raise ValueError(
                f"Adapter with name '{adapter_name}' does not exist."
            )
...
class AdaptedAttention(nn.Module):
    ...
    def __init__(self, model_type: str, adapter_len: int, model):
        ...
        self.adaption_prompt = nn.Parameter(
            torch.empty(
                1, adapter_len, self.model.hidden_size, device=device
            ).normal_()
        )
    ...
    def forward(self, **kwargs):
        ...
        if kwargs.get("output_attention", False):
            raise NotImplementedError(
                "output_attention is not currently supported."
            )
        ...
        adapter_k = (
            key.view(
                1, self.adapter_len, self.model.num_heads, self.model.head_dim
            )
            .repeat(bsz, 1, 1, 1)
            .transpose(1, 2)
        )
        ...
        # (bsz, num_heads, q_len, adapter_len)
        scores = self.adaption_gate * F.softmax(
            scores, dim=-1, dtype=torch.float32
        ).to(query_states.dtype)
        # (bsz, q_len, num_heads * head_dim)
        adapter_output = (
            torch.matmul(scores, adapter_v)
            .transpose(1, 2)
            .reshape(bsz, q_len, -1)
        )
