...
class AdaLoraConfig(LoraConfig):
    ...
    total_step: int | None = field(default=None, metadata={"help": "The total training steps."})
    rank_pattern: dict | None = field(default=None, metadata={"help": "The saved rank pattern."})
...
class AdaLoraModel(LoraModel):
    ...
    **Attributes**:
    ...
    def forward(self, *args, **kwargs):
        ...
        if hasattr(outputs, "loss") and orth_reg_weight > 0:
            regu_loss = 0
            num_param = 0
            for n, p in self.model.named_parameters():
                if ("lora_A" in n or "lora_B" in n) and self.trainable_adapter_name in n:
                    para_cov = p @ p.T if "lora_A" in n else p.T @ p
                    identity_matrix = torch.eye(*para_cov.size(), out=torch.empty_like(para_cov))
                    identity_matrix.requires_grad = False
                    num_param += 1
                    regu_loss += torch.norm(para_cov - identity_matrix, p="fro")
            if num_param > 0:
        ...
        #     logger.info(
        #         f"[DEBUG] Post-Forward Alloc: {torch.cuda.memory_allocated() / 1024**3:.2f} GB, Reserved: {torch.cuda.memory_reserved() / 1024**3:.2f} GB"
        #     )
...
class AdaLoraLayer(LoraLayer):
    def __init__(
        ...
        # AdaLoraLayer uses a different structure, initialize LoraLayer with dummy values
        # The actual parameters are managed via ParameterDicts
        super().__init__(r=0, lora_alpha=1, lora_dropout=0.0, merge_weights=False)
        # Override the single-value attributes with dicts for multi-adapter support
        self.r = {}
        self.lora_alpha = {}
        self.scaling = {}
        ...
        # Store in/out features for use in update_layer
...
class RankAllocator:
    ...
    def update_ipt(self, model):
        # DEBUG: Check memory before IPT update
        # if torch.cuda.is_available():
        #     logger.info(f"[DEBUG] Pre-IPT Update Alloc: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
        ...
        for n, p in model.named_parameters():
            if "lora_" in n and self.adapter_name in n:
                ...
                with torch.no_grad():
                    if p.grad is None:
                        # print(f"[WARN] Gradient None for {n}")
                        continue
        ...
        # if torch.cuda.is_available():
        #     logger.info(f"[DEBUG] Post-IPT Update Alloc: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
