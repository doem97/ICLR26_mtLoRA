    ...
    def add_adapter(self, adapter_name, config=None):
        ...
        if len(self.peft_config) > 1 and self.peft_config[adapter_name].bias != "none":
            raise ValueError(
                ...
                "When multiple adapters, set bias 'none' for all adapters."
            )
...
class MMOELoraLayer(LoraLayer):
    def __init__(self, in_features: int, out_features: int, expert_num: int):
        # MMOELoraLayer uses a different structure, initialize LoraLayer with dummy values
        # The actual parameters are managed via ModuleDicts for multi-adapter support
    ...
    def reset_lora_parameters(self, adapter_name):
        if adapter_name in self.lora_A:
            ...
            for i in range(self.expert_num):
                nn.init.normal_(
                    self.lora_A[adapter_name].loraA[i].mlp.weight,
                    mean=0.0,
                    std=0.01,
                )
...
class MMOELoraLinear(nn.Linear, MMOELoraLayer):
    ...
    def merge(self, task_id):
        if self.active_adapter not in self.lora_A:
            return
        if self.merged:
            warnings.warn("Already merged. Nothing to do.", stacklevel=2)
            return
        if self.r[self.active_adapter] > 0:
            expert_weight = self.lora_gate[self.active_adapter](self.lora_task_embedding[self.active_adapter](task_id))
            for i in range(self.expert_num):
                lora_A_weights = self.lora_A[self.active_adapter].loraA[i].mlp.weight  # noqa: N806
                lora_B_weights = self.lora_B[self.active_adapter].loraB[i].mlp.weight  # noqa: N806
                self.weight.data += (
                    transpose(
    ...
    def unmerge(self, task_id):
        if self.active_adapter not in self.lora_A:
            return
        if not self.merged:
            warnings.warn("Already unmerged. Nothing to do.", stacklevel=2)
            return
        if self.r[self.active_adapter] > 0:
            expert_weight = self.lora_gate[self.active_adapter](self.lora_task_embedding[self.active_adapter](task_id))
            for i in range(self.expert_num):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.active_adapter not in self.lora_A:  # No adapter, directly use linear
        ...
        if self.disable_adapters:  # No adapter
            ...
            # Set to None as MMOELoRA doesn't use balance loss by default
            self.last_blc = None
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
